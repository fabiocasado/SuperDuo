...
public class ScoresAdapter extends CursorAdapter {
    ...
    public void bindView(View view, final Context context, Cursor cursor) {
        boolean isRtl = context.getResources().getBoolean(R.bool.rtl);

        final ViewHolder mHolder = (ViewHolder) view.getTag();
        mHolder.home_name.setText(cursor.getString(COL_HOME));
        mHolder.away_name.setText(cursor.getString(COL_AWAY));
        mHolder.date.setText(cursor.getString(COL_MATCHTIME));
        mHolder.score.setText(Utilies.getScores(context, cursor.getInt(isRtl ? COL_AWAY_GOALS : COL_HOME_GOALS), cursor.getInt(isRtl ? COL_HOME_GOALS : COL_AWAY_GOALS)));
        mHolder.match_id = cursor.getLong(COL_ID);
